...
namespace Casper
{
    ...
    public class Spectrum : Z80 
    {
        ...
        public int _RomType = 0;

        private byte ayRegisterIndex;

        public Spectrum() : base(3.5) 
        ...
        {
            if ((port & 0x0001) == 0)
                Border.SetBorder((ColorIndex16)(outByte & 0x07));
            ...
            if (port == 0xFFFD)
                Sound.WriteAYRegister(ayRegisterIndex, (byte)outByte);

            if (port == 0xBFFD)
                ayRegisterIndex = (byte)outByte;
        }
        ...
        public override void pokeb(int addr, int newByte) 
        {
            ...
            {
                Screen.SetMemory(addr, (byte)newByte);
                base.pokeb(addr, newByte);
            }
        }
        ...
        public override void pokew(int addr, int word) 
        {
            ...
            {
                Screen.SetMemory(addr, (byte)newByte0);
            ...
            if (++addr != (22528 + 768)) 
            {
                ...
                {
                    Screen.SetMemory(addr, (byte)newByte1);
