...
namespace Casper 
{
    public class Keyboard 
    {
        ...
        private Key[] lastPhysicalKeys = Array.Empty<Key>();
        ...
        public void ProcessLogicalKeys() 
        {
            if (logicalKeyQueue.Count > 0)
            {
                ...
                if (lastPhysicalKeys.Length > 0)
                {
                    OnPhysicalKeys(false, lastPhysicalKeys);
                    lastPhysicalKeys = Array.Empty<Key>();
                }

                // Press new keys
                if (logicalKey != char.MinValue)
                {
                    OnPhysicalKeys(true, physicalKeys);
                    lastPhysicalKeys = physicalKeys;
                }
            }
        }
